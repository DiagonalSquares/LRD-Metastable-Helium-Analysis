...
abundance_ratio = 0.01#json.load(file)
...
plt.title(r"Flux Ratios (He I $\lambda10830$ / H I Paschen $\gamma$)")
...
for i in range(len(labels)):
    texts.append(plt.text(nearest_temeperatures[i], nearest_densities[i], labels[i]))
...
if not os.path.exists(path):
        os.makedirs(path)
plt.savefig(str(path) + "/predicted-ratios-higher-range")    
...
for i in range(len(positive_errors)):
    positive_errors[i] = positive_errors[i] * measured_ratios[i]
    negative_errors[i] = negative_errors[i] * measured_ratios[i]
...
plt.savefig(str(path) + "/density-lines-higher-range")    
